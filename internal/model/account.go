...
func NewAccount(id int64, companyID *int64) *Account {
	return &Account{Account: &database.Account{ID: id, CompanyID: companyID}}
}
...
func accountsOrError(accounts []*database.Account, err error, cacheAccounts bool) ([]*Account, error) {
	if err != nil {
		return nil, err
	}
