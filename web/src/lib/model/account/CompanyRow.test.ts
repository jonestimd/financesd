...
describe('CompanyRow', () => {
    ...
    describe('get version', () => {
        it('returns data version', () => {
            expect(new CompanyRow({id: '', version: 1}).version).toEqual(1);
        });
        it('defaults to 0', () => {
            expect(new CompanyRow({id: ''}).version).toEqual(0);
        });
    });
    describe('get name', () => {
        it('returns data.name if no new name', () => {
            expect(new CompanyRow({id: '', name: 'first name'}).name).toEqual('first name');
        });
        it('returns empty string if no data.name and no new name', () => {
            expect(new CompanyRow({id: ''}).name).toEqual('');
        });
        it('returns new name', () => {
            const model = new CompanyRow({id: ''});
    ...
    describe('setName', () => {
        ...
        it('undoes name change', () => {
            const model = new CompanyRow({id: '', version: 1, name: 'old name'});
            model.setName('new name');
    ...
    describe('get accounts', () => {
        it('defaults to 0', () => {
            expect(new CompanyRow({id: ''}).accounts).toEqual(0);
        });
        it('returns accounts length', () => {
            expect(new CompanyRow({id: '', accounts: [newAccountModel(), newAccountModel()]}).accounts).toEqual(2);
        });
    });
    describe('isChanged', () => {
        it('returns true if no version', () => {
            expect(new CompanyRow({id: ''}).isChanged).toBe(true);
        });
        it('returns true if new name', () => {
            const model = new CompanyRow({id: '', version: 1});
            model.setName('new name');
