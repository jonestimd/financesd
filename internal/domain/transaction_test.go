...
func Test_GetTransactions(t *testing.T) {
	...
	t.Run("returns transactions", func(t *testing.T) {
		sqltest.TestInTx(t, func(mockDB sqlmock.Sqlmock, tx *sql.Tx) {
			...
			getTransactionsStub := mocka.Function(t, &getTransactions, []*table.Transaction{expectedTx.Transaction}, nil)
			defer getTransactionsStub.Restore()
...
func Test_GetDetails(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			transaction := &Transaction{Transaction: &table.Transaction{ID: txID}, source: test.source}
